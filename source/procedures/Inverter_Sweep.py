...
def run(parameters, smu_systems, arduino_systems, share=None):
	...
	dlu.saveJSON(dlu.getDeviceDirectory(parameters), is_parameters['saveFileName'], jsonData, subDirectory=parameters['experimentSubFolder']+str(parameters['startIndexes']['experimentNumber']))
...
def runInverterSweep(smu_sweep, inputVoltageMinimum, inputVoltageMaximum, stepsInVINPerDirection, pointsPerVIN, inputVoltageRamps, delayBetweenMeasurements, share=None):
	...
	for direction in range(len(inputVoltages)):
		for (VINi, inputVoltage) in enumerate(inputVoltages[direction]):
			...
			[livePlotter.createLiveDataPoint(plotID='Voltage Transfer Characteristic', 
											labels=['Output Voltage'],
											xValues=[preppedInputVoltage], 
											...
											timeseries=False),
			 livePlotter.createLiveDataPoint(plotID='Output Voltage vs. Time', 
											labels=['Output Voltage'],
											xValues=[timestamp], 
											yValues=[measurement['V_ds']],
											xAxisTitle='Time (s)', 
