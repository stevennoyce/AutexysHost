...
def run(parameters, smu_systems, arduino_systems, share=None):
	...
	for drainVoltage in rt_params['drainVoltages']:
		...
		dlu.saveJSON(dlu.getDeviceDirectory(parameters), rt_params['saveFileName'], jsonData, subDirectory=parameters['experimentSubFolder']+str(parameters['startIndexes']['experimentNumber']))
	
	return jsonData

# === Data Collection ===
def runNoiseCollection(smu_instance, measurementSpeed, drainVoltage, gateVoltages, pointsPerSetpoint, share=None):
	Vds = drainVoltage
	Vgs = gateVoltages[0]
