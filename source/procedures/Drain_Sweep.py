...
def run(parameters, smu_systems, arduino_systems, share=None):
	...
	dlu.saveJSON(dlu.getDeviceDirectory(parameters), ds_parameters['saveFileName'], jsonData, subDirectory=parameters['experimentSubFolder']+str(parameters['startIndexes']['experimentNumber']))
...
def runDrainSweep(smu_instance, isFastSweep, fastSweepSpeed, gateVoltageSetPoint, drainVoltageMinimum, drainVoltageMaximum, stepsInVDSPerDirection, pointsPerVDS, drainVoltageRamps, delayBetweenMeasurements, share=None):
	...
	if(isFastSweep):
		...
		measurements = smu_instance.takeSweep(drainVoltageMinimum, drainVoltageMaximum, gateVoltageSetPoint, gateVoltageSetPoint, points=stepsInVDSPerDirection*2*pointsPerVDS, triggerInterval=triggerInterval, src1vals=drainVoltageList, src2vals=gateVoltageList)
