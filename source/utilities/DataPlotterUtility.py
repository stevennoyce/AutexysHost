...
plotDefinitions = {}
plotDefinitionsBasePath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'PlotDefinitions/') if(not getattr(sys, 'frozen', False)) else (os.path.join(sys._MEIPASS, 'utilities/PlotDefinitions/'))
...
def getPlotTypesFromDependencies(availableDataFiles, plotCategory='device', maxPriority=float('inf')):
	...
	# Convert 'plotTypes' to a list of dictionaries containing key info on each plot
	plotTypes = [{'type':plotType, 'name':plotDefinitions[plotType]['description']['name'] if('name' in plotDefinitions[plotType]['description']) else ''} for plotType in plotTypes]
