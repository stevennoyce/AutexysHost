...
def plot(deviceHistory, identifiers, mode_parameters=None):
	...
	for i in range(len(deviceHistory)):
		for j in range(len(deviceHistory[i]['Results']['timestamps'])):
			line = ax.plot(np.array(deviceHistory[i]['Results']['timestamps'][j]) - min(deviceHistory[i]['Results']['timestamps'][0]),  np.array(deviceHistory[i]['Results']['id_data'][j])*current_scale, color=colors[i], marker='o', markersize=2, linewidth=1)
