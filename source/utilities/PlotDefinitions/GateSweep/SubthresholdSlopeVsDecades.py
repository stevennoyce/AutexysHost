...
def plot(deviceHistory, identifiers, mode_parameters=None):
	...
	abs_min_value = min(max_value, abs(min_value)) if(mode_parameters['yscale'] is None) else float(mode_parameters['yscale'])
	...
	for i in range(len(deviceHistory)):
		line = plotSubthresholdCurveSlope(ax, deviceHistory[i], colors[i], direction=mode_parameters['sweepDirection'], x_axis='drain current', absoluteXaxis=True, scaleYaxisBy=yscale, lineStyle=None, errorBars=mode_parameters['enableErrorBars'])
