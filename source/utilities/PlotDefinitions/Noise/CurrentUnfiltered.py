...
def plot(deviceHistory, identifiers, mode_parameters=None):
	...
	for i in range(len(gateVoltages)):
		line = ax.plot(np.array(currentAverages[i]) * current_scale, np.array(unfilteredNoise[i]) * noise_scale, color=colors[i], marker='o', markersize=4, linewidth=1, linestyle=None)
