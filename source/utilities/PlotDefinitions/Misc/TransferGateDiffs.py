...
plotDescription = {
	...
	'plotDefaults': {
		...
		'neg_label':'$-I_{{D}}$ ($\\mathregular{\\mu}$A)',
		'ii_label':'$I_{{D}}$, $I_{{G}}$ ($\\mathregular{\\mu}$A)',
		'neg_ii_label':'$-I_{{D}}$, $I_{{G}}$ ($\\mathregular{\\mu}$A)',
...
def plot(deviceHistory, identifiers, mode_parameters=None):
	...
	colors = setupColors(fig, len(deviceHistory), colorOverride=mode_parameters['colorsOverride'], colorDefault=plotDescrip_current['plotDefaults']['colorDefault'], colorMapName=plotDescrip_current['plotDefaults']['colorMap'], colorMapStart=0.7, colorMapEnd=0, enableColorBar=mode_parameters['enableColorBar'], colorBarTicks=[0,0.6,1], colorBarTickLabels=[totalTime, holdTime, '$t_0$'], colorBarAxisLabel='')		
	...
	for i in range(len(deviceHistory)):
		ax.plot(deviceHistory[i]['Results']['vgs_data'][0], deviceHistory[i]['Results']['id_data'][0])
		ax.plot(deviceHistory[i]['Results']['vgs_data'][1], deviceHistory[i]['Results']['id_data'][1])
